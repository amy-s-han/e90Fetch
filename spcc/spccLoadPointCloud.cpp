...
class SPCCLoadPCDemo: public MzGlutApp {
public:
  ...
  SPCCLoadPCDemo(int argc, char** argv, const std::vector<ccdw::vec3> pc):
    MzGlutApp(argc, argv) 
  {
...
bool loadPointCloud(std::string filename, std::vector<ccdw::vec3> *pointCloud){
  std::ifstream f(filename.c_str());
  std::string line;
  float x, y, z;

  // float xMax = 0, xMin = 100, yMax = 0, yMin = 100, zMax = 0, zMin = 100;
  ...
  if(f.is_open()){
    while(f >> x >> y >> z){
      ...
      pointCloud->push_back(ccdw::vec3(y, z, -x ));
    }

    // std::cout << "xMax: " << xMax << ", xMin: " << xMin << ", yMax: " << yMax 
     // << ", yMin: " << yMin << ", zMax: " << zMax << ", zMin: " << zMin << std::endl;
...
int main(int argc, char** argv) {
  ...
  if(argc == 2){
    res = loadPointCloud(argv[1], &pointCloud);
  } else {
    res = loadPointCloud("../mzcommon/pointcloud.txt", &pointCloud);
  }

  if(!res){
    std::cout << "Could not load point cloud data. Exiting." << std::endl;
    return 1;
  }

  std::cout << "Point cloud size: " << pointCloud.size() << std::endl;

  SPCCLoadPCDemo demo(argc, argv, pointCloud);
  demo.run();
